...
namespace Concise
{
	...
	void Vertices::Submit(std::vector<Vertex>& vertexData, std::vector<UInt32>& indexData)
	{
		...
		Utils::VK_CHECK_RESULT(vkCreateBuffer(m_device->GetLogicalDevice(), &vertexBufferCreateInfo, nullptr, &m_vertexBuffer.buffer));
		...
		Utils::VK_CHECK_RESULT(vkAllocateMemory(m_device->GetLogicalDevice(), &memAlloc, nullptr, &staginIndices.memory));
		Utils::VK_CHECK_RESULT(vkMapMemory(device, staginIndices.memory, 0, indexData.size(), 0, &data));
		memcpy(data, indexData.data(), indexData.size());
		vkUnmapMemory(device, staginIndices.memory);
		Utils::VK_CHECK_RESULT(vkBindBufferMemory(device, staginIndices.buffer, staginIndices.memory, 0));

		/** destination buffer */
		indexBufferCreateInfo.usage = VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
		Utils::VK_CHECK_RESULT(vkCreateBuffer(m_device->GetLogicalDevice(), &indexBufferCreateInfo, nullptr, &m_indexBuffer.buffer));
		vkGetBufferMemoryRequirements(m_device->GetLogicalDevice(), m_indexBuffer.buffer, &memReqs);
		memAlloc.allocationSize = memReqs.size;
		memAlloc.memoryTypeIndex = getMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		Utils::VK_CHECK_RESULT(vkAllocateMemory(device, &memAlloc, nullptr, &m_indexBuffer.memory));
		Utils::VK_CHECK_RESULT(vkBindBufferMemory(device, m_indexBuffer.buffer, m_indexBuffer.memory, 0));
		...
		vkCmdCopyBuffer(copyCmd, stagingVertices.buffer, vertices.buffer, 1, &copyRegion);
		
		copyRegion.size = indexData.size();
		vkCmdCopyBuffer(copyCmd, staginIndices.buffer, indices.buffer,	1, &copyRegion);
